...
export const useKeyframes = ({
  ...
  const calculateFrameCount = React.useCallback(() => {
    if (!containerRef || !containerRef.current) return 10;
    ...
    try {
      ...
      const dimensions = await new Promise<{ width: number; height: number }>(
        (resolve, reject) => {
          ...
          const onError = (e: ErrorEvent | Event | any) => {
            const errorMessage = e?.message || e?.error?.message || 'Unknown error';
            reject(new Error(`Failed to load video metadata: ${errorMessage}`));
            cleanup();
          };
        ...
        const attemptLoad = () => {
          ...
          const onError = (e: ErrorEvent | Event | any) => {
            ...
            if (loadAttempts < MAX_LOAD_ATTEMPTS) {
              ...
              reject(
                new Error(
                  `Video load failed after ${MAX_LOAD_ATTEMPTS} attempts: ${errorMessage}`
                )
              );
            }
          };
          ...
          while (retryCount < MAX_RETRIES && !frameExtracted) {
            try {
              ...
              const seekPromise = new Promise<void>((resolve, reject) => {
                ...
                const onError = (error: ErrorEvent | Event | any) => {
                  clearTimeout(seekTimeout);
                  video!.removeEventListener("seeked", onSeeked);
                  video!.removeEventListener("error", onError);
                  const errorMessage = error?.message || error?.error?.message || 'Unknown seek error';
                  reject(new Error(`Seek error: ${errorMessage}`));
                };
