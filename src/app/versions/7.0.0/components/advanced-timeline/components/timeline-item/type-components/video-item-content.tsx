...
export const VideoItemContent: React.FC<VideoItemContentProps> = ({
  ...
  const isShowingThumbnails = thumbnails && thumbnails.length > 0 && itemWidth > 40 && !isLoadingThumbnails; // Reduced threshold from 60 to 40px
  ...
  if (isShowingThumbnails) {
    const thumbnailWidth = Math.max(30, itemWidth / thumbnails.length); // Reduced minimum from 40 to 30px for more thumbnails
      ...
      <div className="flex items-center h-full w-full overflow-hidden relative rounded-[3px]">
        ...
        <div className="flex-1 flex h-full">
          {thumbnails.map((thumbnail, index) => (
            <div
              key={index}
              className="flex-shrink-0 h-full last:border-r-0 overflow-hidden"
