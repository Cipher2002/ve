...
export const useTimelineDragAndDrop = ({
      ...
      if (action === "resize-start" || action === "resize-end") {
        ...
        if (dragInfo && dragInfo.selectedItemsSnapshot.length > 0) {
          ...
          if (primaryItem && (primaryItem.type === 'video' || primaryItem.type === 'audio') && primaryItem.mediaSrcDuration !== undefined) {
            ...
            const maxAllowedDuration = primaryItem.mediaSrcDuration - calculatedMediaStart;
  ...
  const handleDrag = useCallback(
    (clientX: number, clientY: number) => {
      ...
      switch (dragInfo.action) {
        ...
        case "resize-end":
          ...
          if (primaryItemEnd && (primaryItemEnd.type === 'video' || primaryItemEnd.type === 'audio') && primaryItemEnd.mediaSrcDuration !== undefined) {
            ...
            const maxAllowedDuration = primaryItemEnd.mediaSrcDuration - mediaStartOffset;
      ...
      if (isMultiDrag) {
        ...
        for (const snapshot of dragInfo.selectedItemsSnapshot) {
          if (snapshot.id !== dragInfo.id) {
            ...
            if (itemNewStart + itemDuration > totalDuration) {
              const rightConstraint = totalDuration - itemDuration - snapshot.originalStart;
              constrainedDeltaTime = Math.min(constrainedDeltaTime, rightConstraint);
            }
          }
        }
        ...
        for (const snapshot of dragInfo.selectedItemsSnapshot) {
          ...
          if (snapshot.id === dragInfo.id) {
            ...
            itemNewStart = Math.max(0, Math.min(totalDuration - finalDuration, snapshot.originalStart + deltaTime));
            ...
            itemNewStart = Math.max(0, Math.min(totalDuration - snapshot.originalDuration, snapshot.originalStart + deltaTime));
      ...
      const yWithinTracks = yWithinContent - TIMELINE_CONSTANTS.MARKERS_HEIGHT;
