...
export const useNewItemDrag = ({
  ...
  const { setGhostElement, setNewItemDragState } = useTimelineStore();
  ...
  }, [setGhostElement, setNewItemDragState]);
  ...
  const newItemIsValidDrop = useMemo(() => {
    return lastPositionRef.current?.isValid ?? true;
  }, [lastPositionRef.current?.isValid]);
