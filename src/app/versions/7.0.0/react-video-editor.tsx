...
export default function ReactVideoEditor({ projectId, isAdminMode = false }: { projectId: string; isAdminMode?: boolean }) {
  ...
  const applyCrop = useCallback(async (overlayId: number) => {
    ...
    // Apply CSS-based cropping by keeping the cropData
    // The actual cropping will be handled in the layer content components
    changeOverlay(overlayId, {
      ...cropableOverlay,
      // Keep the cropData - this will be used for CSS-based cropping
      cropData: cropableOverlay.cropData,
    });
